...
async function handleRequest(request: NextRequest, params: { slug: string[] }, method: string) {
  if (process.env.CI !== 'true') {
    return NextResponse.json({ error: 'API not available in dev mode' }, { status: 404 });
  }
  ...
  if (method === 'POST' && path === 'auth/register') {
    return NextResponse.json({ id: `user_${Date.now()}`, ...body }, { status: 201 });
  }
  ...
  if (method === 'POST' && path === 'homes') {
    return NextResponse.json({ id: `home_${Date.now()}`, ...body }, { status: 201 });
  }
  ...
  if (method === 'POST' && path.includes('item')) {
    return NextResponse.json({ id: `item_${Date.now()}`, ...body }, { status: 201 });
  }
